...
export async function POST(
  ...
  if (event.status !== "Requested") {
    return NextResponse.json(
      { error: `Cannot approve a submission with status "${event.status}"` },
      { status: 400 }
    );
  }

  const result = await sql`
    UPDATE events SET status = 'Planned', updated_at = NOW() WHERE id = ${id} RETURNING *
  `;
  ...
  if (event.submitter_email) {
    const templateId = process.env.HUBSPOT_TEMPLATE_APPROVED;
    if (!templateId) {
      emailError = "HUBSPOT_TEMPLATE_APPROVED not set";
    } else {
