...
export const columns: ColumnDef<TFEvent>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const statusColors: Record<string, string> = {
        ...
        "Not Attending": "bg-gray-200 text-gray-700",
      };
      return (
  ...
  },
  {
    ...
    header: ({ column }) => <SortHeader column={column} label="Abstract" />,
    cell: ({ row }) => {
      const s = row.original.abstract_status;
      if (!s) return <span className="text-slate-400">—</span>;
      const abstractColors: Record<string, string> = {
      ...
      return (
        <span className={`inline-block px-2 py-0.5 rounded text-xs font-medium whitespace-nowrap ${abstractColors[s] || "bg-gray-100 text-gray-800"}`}>
          {s}
        </span>
      );
    },
